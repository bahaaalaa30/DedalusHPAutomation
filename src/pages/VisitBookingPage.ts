...
export class VisitBookingPage {
  ...
  private clinicButton = this.page.locator('#clinic-btn');
  private clinicSelection = this.page.locator('#clinic-list > div.clinic-list > div:nth-child(25)');
  private practitionerSelection = this.page.locator('app-crm-quick-filters .quick-filter-list > div:nth-child(2) .filter-name');
  private patientSearchInput = this.page.locator("input[placeholder*='Search Patient'], .appt-component input").first();
  private searchButton = this.page.locator('app-ex-identify-patient span img').first();
  private searchResult = this.page.locator('app-find-patient-detail .patients-list .list-content > div').first();
  private continueButton = this.page.getByRole('button', { name: 'Continue' }).first();
  private paymentButton = this.page.getByRole('button', { name: 'Payment' }).first();
  private cashField = this.page.locator('app-ex-visit-payment-details input').first();
  private createVisitButton = this.page.locator('app-ex-create-visit .ex-book-appt-footer button').last();
  private doneButton = this.page.locator('app-ex-create-visit button.primary-button').last();
  private patientId = this.page.locator('app-find-patient-detail .patients-list .list-content > div').first();
  private noResults = this.page.getByText('لم يتم العثور على نتائج');
  private languageMenu = this.page.locator('#language-menu');
  private arabicLanguage = this.page.getByText('عربى', { exact: true });
  private globalSearch = this.page.locator('app-crm-header input').first();
  private patientIdInput = this.page.locator("input[placeholder='Enter Patient ID']");
  private findButton = this.page.getByRole('button', { name: 'Find', exact: true });
  private firstName = this.page.locator('#first_name');
  private nationalId = this.page.locator("input[placeholder*='National ID']");
  private male = this.page.getByText('Male', { exact: false }).first();
  private female = this.page.getByText('Female', { exact: false }).first();
  private approvedStatus = this.page.locator('.approvals-queue-status').filter({ hasText: 'Approved' });
  ...
  async bookNextAvailableTimeSlot() { const slots=this.page.locator('p').filter({hasText:/^(0?[1-9]|1[0-2]):[0-5][0-9]\s?(AM|PM)$/i}); await slots.first().waitFor({state:'visible'}); await slots.first().click(); return this; }
  ...
  async CancelBookedVisit() { await this.page.getByText('Visit Cancellation', {exact:true}).click(); await this.page.getByRole('button',{name:'Continue'}).last().click(); }
  async CancelAppointment() { await this.page.getByText('Cancel Appointment',{exact:true}).click(); await this.page.getByText('Mistake in entry',{exact:true}).click(); await this.page.getByRole('button',{name:'Continue'}).last().click(); }
  ...
  async selectPayerFacility() { await this.page.locator('#facility-menu').click(); await this.page.locator('#facility-menu-actions > div > div > div > div').last().click(); }
  ...
  async sendRequestAndWaitForResponseNoneligible() { await this.page.getByRole('button',{name:/Proceed|Continue|Check Eligibility/i}).last().click().catch(()=>{}); }
  ...
  async PayBill(patientName:string) { await this.BillPage(patientName); await this.page.getByText('Payment',{exact:true}).click().catch(()=>{}); }
}
